...
interface Props { caseFile: FactCheckCase; selected: string[]; onToggle: (id: string) => void; onContinue: () => void; onBack: () => void }
export function ClaimAnalyzer({ caseFile, selected, onToggle, onContinue, onBack }: Props) {
  ...
  return <section className="page-section" data-od-id="claim-analyzer">
    ...
    <div className="action-row"><PreviousButton onClick={onBack} /><span>{selected.length}/{required.length}개 확인</span><button className="button button-primary" disabled={!complete} onClick={onContinue}>자료 살펴보기<ArrowRight aria-hidden="true" /></button></div>
  </section>;
}
