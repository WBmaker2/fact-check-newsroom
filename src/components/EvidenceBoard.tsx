...
interface Props {
  ...
  onToggleEvidence: (sourceId: string) => void; onContinue: () => void; onBack: () => void; late?: boolean;
}

export function EvidenceBoard({ atoms, sources, classified, selectedSources, onClassify, onToggleEvidence, onContinue, onBack, late = false }: Props) {
  ...
  const complete = chosen.length > 0 && chosen.every((source) => checkable.every((atom) => classified[source.id]?.[atom.id]));
  const accurate = complete && chosen.every((source) => checkable.every((atom) => classified[source.id]?.[atom.id] === source.assessments[atom.id]));
  const total = chosen.length * checkable.length;
  const classifiedCount = chosen.reduce((count, source) => count + checkable.filter((atom) => classified[source.id]?.[atom.id]).length, 0);
  ...
  return <section className="page-section" data-od-id={late ? 'late-evidence' : 'evidence-board'}>
    ...
    <div className="evidence-stack">{sources.map((source) => {
      ...
      {!selected ? <p className="evidence-choice-note">판정에 도움이 된다고 생각하면 위 버튼을 누른 뒤 비교하세요.</p> : checkable.map((atom) => {
    ...
    <div className="action-row"><PreviousButton onClick={onBack} label={late ? '새 자료 다시 보기' : '자료 단서 다시 보기'} /><span>비교 {classifiedCount}/{total || 2} · 도움 된 자료 {selectedSources.length}/2개</span><button className="button button-primary" disabled={!complete} onClick={submit}>{late ? '새 자료로 다시 판단하기' : '첫 번째 판단하기'}<ArrowRight aria-hidden="true" /></button></div>
  </section>;
}
