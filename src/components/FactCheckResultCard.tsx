import { Check, FileCheck2, Printer, RotateCcw } from 'lucide-react';
...
interface Props { caseFile: FactCheckCase; sources: SourceCard[]; first: Verdict; final: Verdict; headline: string; selectedSourceIds: string[]; onRestart: () => void; onBack: () => void; onPrint: () => void }
export function FactCheckResultCard({ caseFile, sources, first, final, headline, selectedSourceIds, onRestart, onBack, onPrint }: Props) {
  ...
  return <section className="page-section result-page" data-od-id="result-card"><header className="section-heading"><p className="context-line">활동 완료</p><h1>팩트체크 결과 카드</h1><p>주장과 자료를 차근차근 비교한 결과예요.</p></header>
    ...
    <div className="action-row result-actions"><PreviousButton onClick={onBack} label="제목 다시 고르기" /><span>결과 카드는 읽기 전용이며 서버에 저장하지 않고 이 기기에서만 인쇄해요.</span><div className="action-buttons"><button className="button button-secondary" onClick={onPrint}><Printer aria-hidden="true" />결과 카드 인쇄</button><button className="button button-secondary" onClick={onRestart}><RotateCcw aria-hidden="true" />다른 사건 해 보기</button></div></div>
  </section>;
}
