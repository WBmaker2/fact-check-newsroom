...
const labels: Record<Verdict, string> = { confirmed: '자료로 확인됨', 'partly-confirmed': '일부만 확인됨', contradicted: '제시된 자료와 맞지 않음', insufficient: '근거가 부족해 판단 보류' };
...
export function FactCheckResultCard({ caseFile, sources, first, final, headline, selectedSourceIds, onRestart }: Props) {
  ...
  return <section className="page-section result-page" data-od-id="result-card"><header className="section-heading"><p className="context-line">편집 완료</p><h1>팩트체크 결과 카드</h1><p>사람이 아니라 주장과 자료의 관계를 검토한 결과입니다.</p></header>
    ...
    <div className="action-row"><span>결과 카드는 읽기 전용이며 저장하거나 공유하지 않습니다.</span><button className="button button-secondary" onClick={onRestart}><RotateCcw aria-hidden="true" />다른 사건 편집하기</button></div>
  </section>;
}
